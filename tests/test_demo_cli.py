...
def test_demo_default_writes_ndjson_and_prints_card(tmp_path, monkeypatch):
...
def test_demo_no_save_skips_ndjson(tmp_path, monkeypatch):
    env = {"HOME": str(tmp_path), "PATH": __import__("os").environ.get("PATH", ""),
           "NO_COLOR": "1"}
...
def test_demo_workflow_flag(tmp_path, monkeypatch):
    env = {"HOME": str(tmp_path), "PATH": __import__("os").environ.get("PATH", ""),
           "NO_COLOR": "1"}
